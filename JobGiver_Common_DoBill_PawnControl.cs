...
namespace emitbreaker.PawnControl
{
    ...
    /// </summary>
    public abstract class JobGiver_Common_DoBill_PawnControl : JobGiver_Scan_PawnControl
    {
        ...
        protected override Job TryGiveJob(Pawn pawn)
        {
            return Utility_JobGiverManager.StandardTryGiveJob<JobGiver_Common_DoBill_PawnControl>(
                ...
                (p, forced) =>
                {
                    ...
                    // Update bill giver cache if needed
                    if (!_lastBillGiverCacheUpdate.TryGetValue(mapId, out int last)
                        || now - last >= CacheUpdateInterval)
                    {
                        _lastBillGiverCacheUpdate[mapId] = now;
                        _billGiversCache[mapId] = new List<Thing>(GetBillGivers(p.Map));
                    }

                    // Get bill givers from cache
                    if (!_billGiversCache.TryGetValue(mapId, out var billGivers) || billGivers.Count == 0)
                        return null;

                    // Filter bill givers to those that are valid for this pawn
                    var validBillGivers = billGivers
                        .Where(thing => IsValidBillGiver(thing, p, forced))
                        .ToList();
        ...
        {
            Utility_CacheManager.ResetJobGiverCache(_billGiversCache, _reachabilityCache);
            _lastBillGiverCacheUpdate.Clear();
            _billFailTicksCache.Clear();
        }

        /// <summary>
        /// Defcount list utility class
        /// </summary>
