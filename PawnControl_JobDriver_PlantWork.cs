...
{
    public abstract class PawnControl_JobDriver_PlantWork : JobDriver
    {
        ...
        protected override IEnumerable<Toil> MakeNewToils()
        {
            ...
            cut.tickAction = delegate
            {
                ...
                // ✅ Safe XP gain only for valid skill pawns
                if (actor.RaceProps.Humanlike && actor.skills != null)
                {
                ...
                if (workDone >= plant.def.plant.harvestWork)
                {
                    ...
                    {
                        StatDef stat = (plant.def.plant.harvestedThingDef.IsDrug || plant.def.plant.drugForHarvestPurposes)
                            ? StatDefOf.DrugHarvestYield
                            : StatDefOf.PlantHarvestYield;

                        float statValue = stat != null? actor.GetStatValue(stat) : 1 ;
                        ...
                        else
                        {
                            ...
                            if (num > 0)
                            {
                                ...
                                // ✅ Safe record logging only if available
                                if (actor.RaceProps.Humanlike && actor.records != null)
                                {
                            ...
                            if (plant.HarvestableNow)
                            {
                                ...
                                {
                                    List<ThingDefCountClass> yieldList = allComp.GetAdditionalHarvestYield()?.ToList();
                                    if (yieldList != null)
                                    {
                                        for (int i = 0; i < yieldList.Count; i++)
                                        {
                                            Thing thing2 = ThingMaker.MakeThing(yieldList[i].thingDef);
                                            thing2.stackCount = yieldList[i].count;
            ...
            EffecterDef effecter = EffecterDefOf.Harvest_Plant;
            if (Plant != null && Plant.def != null && Plant.def.plant.IsTree)
            {
                effecter = EffecterDefOf.Harvest_Tree;
            }
            cut.WithEffect(effecter, TargetIndex.A);
            ...
            cut.WithProgressBar(TargetIndex.A, () =>
            {
                ...
                return workDone / Plant.def.plant.harvestWork;
            }, true);

            if (Plant != null && Plant.def != null && Plant.def.plant.soundHarvesting != null)
            {
                SoundDef sound = Plant.def.plant.soundHarvesting;
                cut.PlaySustainerOrSound(() => sound);
            }
