    ...
    public static class PatchImpl_MechClusterDefend
    {
        public static void Postfix(object __instance)
        {
            ...
            foreach (var pawn in toil.lord.ownedPawns)
            {
                ...
                if (!Utility_CacheManager.HasTag(pawn.def, NonHumanlikePawnControlTags.AutoDraftInjection)) continue;

                if (Utility_CacheManager.HasTag(pawn.def, "Mech_DefendBeacon") && beaconDuty != null)
                    pawn.mindState.duty = new PawnDuty(beaconDuty, toil.FlagLoc, 18f);
                else if (Utility_CacheManager.HasTag(pawn.def, "Mech_EscortCommander") && escortDuty != null)
